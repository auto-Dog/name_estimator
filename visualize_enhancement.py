...
def calculate_gradient_penalty(real_tuple, fake_tuple, critic):
    ...
    Inputs are two tuples of tensors (real and fake), each containing three tensors. And a critic function
    """
    # Unpack the tuples
    ...
    # Interpolate between real and fake for image
    interpolated_images = eta * real_images + (1 - eta) * fake_images
    interpolated_embeddings = real_embeddings
    ...
    gradients = autograd.grad(
        ...
        inputs=[interpolated_images, interpolated_embeddings],
    ...
    combined_gradients = torch.cat([gradients_images, gradients_embeddings], dim=1)
...
def wgan_train(x:torch.Tensor,patch_id,labels,
    ...
    ''' Train conditional Wasserstein GAN based generator (enhancement net) for one step '''
    ...
    iter_lambda = min(1.0,iter_num/1000) # control ssim
